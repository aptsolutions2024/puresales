...
$chln_id=$_REQUEST['chln_id'];
 ...
 $res=$user->getDeliveredCylinder($cust_id,$chln_date,$chln_id);  
 $i=1;
 $total=sizeof($res);
 if($total!=0){
...
<tr>
<th scope="row">
    <input type="hidden" name="retchlnno[]" id="retchlnno<?=$cylinder['id'];?>" value="<?=$cylinder['chln_no'];?>">
    <input type="hidden" name="cyId[]" id="cyId<?=$cylinder['id'];?>" value="<?=$cylinder['cy_id'];?>">
    <input type="checkbox" name="cylinderRetR[]" id="cylinderRetR<?=$cylinder['id'];?>"  onClick="displyCount()" class="selectItem"value="<?=$cylinder['id'];?>">
    <input type="hidden" name="cyName" id="cyName<?= $cylinder['id'];?>" value="<?= $cylinder['name'];?>">
   <input type="hidden" name="gasID" id="gasID<?= $cylinder['id'];?>" value="<?= $cylinder['gas_id'];?>">
